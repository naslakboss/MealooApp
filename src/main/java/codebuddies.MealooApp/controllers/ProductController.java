...
//        productTypeService.save(dairy);
//        productTypeService.save(grains);
//        productTypeService.save(meat);
...
//                new Macronutrients(3, 4, 4), dairy);
//        Product beef = new Product("Beef", 30, 300,
//                new Macronutrients(30, 5, 30), meat);
...
//     ProductType - MongoDB could not handle ENUM types so i need to make small class with only one parameter
    ...
    ResponseEntity<ProductType> addNewProductType(@RequestBody @Valid ProductType productType) {
        if(productTypeService.existsByName(productType.getProductType())) throw new EntityAlreadyFoundException("Product Type");
        return ResponseEntity.created(URI.create("/" + productType.getProductType())).body(productType);
    }
